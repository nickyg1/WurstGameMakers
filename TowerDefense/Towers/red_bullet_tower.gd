...
func  _process(delta):
	get_node("Upgrade/ProgressBar").scale = Vector2(0.32, 0.32)
	get_node("Upgrade/ProgressBar").global_position = self.position + Vector2(-20, -40)
...
func _on_input_event(viewport, event, shape_idx):
	if event is InputEventMouseButton and event.button_mask == 1:
		var towerPath = get_tree().get_root().get_node("Main/TowersContainer")
		for i in towerPath.get_child_count():
			if towerPath.get_child(i).name != self.name:
				towerPath.get_child(i).get_node("Upgrade/Upgrade").hide()
		get_node("Upgrade/Upgrade").visible = !get_node("Upgrade/Upgrade").visible
		get_node("Upgrade/Upgrade").scale = Vector2(0.32, 0.32)
